...
Examples:
  ...
  teldrive check --concurrent 8`,
	...
	cmd.Flags().Bool("clean-uploads", false, "Clean incomplete uploads")
	cmd.Flags().Bool("clean-pending", false, "Clean files with pending_deletion status")
	return cmd
}
...
func (cp *channelProcessor) process() error {
	cleanPending, _ := cp.cmd.Flags().GetBool("clean-pending")
	cleanUploads, _ := cp.cmd.Flags().GetBool("clean-uploads")
